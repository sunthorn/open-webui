<script lang="ts">
	...
	import { isStale, sectionLabel } from '$lib/apps/clientSections';
...
<section class="flex-1 min-w-0">
	{#if !section}
		<div class="rounded-2xl border border-dashed border-gray-200 dark:border-gray-800 px-4 py-8 text-center">
			<p class="text-sm text-gray-500">Pick a section.</p>
		</div>
	{:else if !rows.length}
		<h2 class="text-base font-semibold mb-2">{sectionLabel(section)}</h2>
		<p class="text-sm text-gray-500">Not read yet — press <span class="font-medium">Read from XPLAN</span>.</p>
	{:else}
				...
				<div class="rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
					...
					{:else}
						<p class="text-sm text-gray-500">
							{section === 'tasks' ? 'No open tasks for this client.' : 'Nothing recorded on this page in XPLAN.'}
						</p>
					{/if}
